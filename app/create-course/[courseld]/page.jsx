...
function CourseLayout({ params }) {
  ...
  const courseld = resolvedParams?.courseld;
  useEffect(() => {
    const userEmail = user?.primaryEmailAddress?.emailAddress;
    if (courseld && userEmail) {
      GetCourse();
    }
  }, [courseld, user?.primaryEmailAddress?.emailAddress]);
  ...
  const GetCourse = async () => {
    const result = await db
      ...
      .where(
        and(
          eq(CourseList.courseId, courseld),
  ...
  return (
    <div>
      <div className="mt-10 px-7 md:px-20 lg:px-44">
        <h2 className="text-2xl text-center font-bold">Course Layout</h2>
